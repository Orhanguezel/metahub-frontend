...
import EnsotekprodPage from "@/modules/ensotekprod/public/pages/Page";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  // seo_snap_${tenant}_ensotekprod_${locale}
  const snapKey = `seo_snap_${tenant}_ensotekprod_${locale}`;
  const snapOldKey = `seo_snap_${tenant}_ensotekprod`;
  const snapRaw =
...
/* ---------- PAGE (RSC) ---------- */
export default async function EnsotekprodRoutePage() {
  const hdrs = await headers();
  const cookieStore = await cookies();
  ...
  return (
    <>
      <SeoFromStore page="ensotekprod" locale={locale} host={hostHeader} />
      <EnsotekprodPage />
    </>
  );
}

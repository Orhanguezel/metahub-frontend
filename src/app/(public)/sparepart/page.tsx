...
import SparepartPage from "@/modules/sparepart/public/pages/Page";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  // seo_snap_${tenant}_sparepart_${locale}
  const snapKey = `seo_snap_${tenant}_sparepart_${locale}`;
  const snapOldKey = `seo_snap_${tenant}_sparepart`;
  const snapRaw =
...
/* ---------- PAGE (RSC) ---------- */
export default async function SparepartRoutePage() {
  const hdrs = await headers();
  const cookieStore = await cookies();
  ...
  return (
    <>
      <SeoFromStore page="sparepart" locale={locale} host={hostHeader} />
      <SparepartPage />
    </>
  );
}

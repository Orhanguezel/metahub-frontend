...
import ContactMePage from "@/modules/contact/public/pages/ContactMePage";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  // seo_snap_${tenant}_contact_${locale}
  const snapKey = `seo_snap_${tenant}_contact_${locale}`;
  const snapOldKey = `seo_snap_${tenant}_contact`;
  const snapRaw =
...
/* ---------- PAGE (RSC) ---------- */
export default async function ContactMeRoutePage() {
  const hdrs = await headers();
  const cookieStore = await cookies();
  ...
  return (
    <>
      {/* Hydration sonrası client tarafı head’i zenginleştirir / günceller */}
      <SeoFromStore page="contact" locale={locale} host={hostHeader} />
      <ContactMePage />
    </>
  );
}

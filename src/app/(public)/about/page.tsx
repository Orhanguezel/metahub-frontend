...
import AboutPage from "@/modules/about/public/pages/Page";
...
/* ---------- SSR METADATA (snapshot fallback) ---------- */
export async function generateMetadata(): Promise<Metadata> {
  ...
  // seo_snap_${tenant}_about_${locale}
  const snapKey = `seo_snap_${tenant}_about_${locale}`;
  const snapOldKey = `seo_snap_${tenant}_about`;
  const snapRaw =
...
export default async function AboutRoutePage() {
  ...
  return (
    <>
      {/* Tek yerde SEO (client) */}
      <SeoFromStore page="about" locale={locale} host={hostHeader} />
      <AboutPage />
    </>
  );
}

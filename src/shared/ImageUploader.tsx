...
export default function ImageUploader({
  ...
  const pickLabel = t("uploader.add", "+ Add Images ({left} left)").replace(
    "{left}",
    String(left)
  );
  ...
  const filterIncoming = useCallback(
    (incoming: File[]) => {
      ...
      for (const f of incoming) {
        ...
        if (!matchesAccept(f)) {
          setError(t("uploader.typeMismatch", "Some files are not allowed by accept filter"));
          continue;
        }
    ...
    [files, left, sizeLimitMB, t, matchesAccept]
  );
  ...
  return (
    <Wrap onPaste={onPaste}>
      ...
      >
        ...
        <DropSub>
          {t("uploader.hint", "Drag & drop, click to browse")}
          {allowPaste ? ` • ${t("uploader.paste", "or paste")}` : ""}
          {helpText ? ` • ${helpText}` : ""}
        </DropSub>
      </Drop>
      ...
      <SmallInfo>
        ...
        {sizeLimitMB
          ? ` • ${t("uploader.sizeLimit", "Max {n}MB/file").replace("{n}", String(sizeLimitMB))}`
          : ""}
      </SmallInfo>
    </Wrap>
  );

...
const getArchives = (articles: IArticles[]) => {
...
const getCategories = (articles: IArticles[], lang: SupportedLocale) => {
  if (!Array.isArray(articles)) return [];
...
export default function ArticlesPage() {
  ...
  Object.entries(translations).forEach(([lng, resources]) => {
    if (!i18n.hasResourceBundle(lng, "articles")) {
      i18n.addResourceBundle(lng, "articles", resources, true, true);
    }
  });
  ...
  const archives = getArchives(articles || []);
  const categories = getCategories(articles || [], lang);
  ...
  if (!articles || articles.length === 0) {
    return (
      <PageWrapper>
        <NoArticles>{t("page.noArticles", "Hiç haber bulunamadı.")}</NoArticles>
      </PageWrapper>
    );
  }
  ...
  return (
    <PageWrapper>
      <MainGrid>
        {/* SOL ANA KOLON */}
        <LeftColumn>
          {filteredArticles.map((item: IArticles) => (
            <ArticlesItem key={item._id}>
              {item.images?.[0]?.url && (
                <MainImageWrap>
  <Image
  ...
  />
              ...
              <ArticlesTitle>
                <Link href={`/articles/${item.slug}`}>{item.title?.[lang] || "Untitled"}</Link>
              </ArticlesTitle>
              <ArticlesMeta>
                {/* <span>
                  <b>{t("author", "Yazar")}: </b>
              ...
              <ReadMoreBtn href={`/articles/${item.slug}`}>
          ...
          <SidebarBox>
            ...
            <SidebarList>
              {recentArticles.length ? (
                recentArticles.map((n) => (
                  <li key={n._id}>
                    <Link href={`/articles/${n.slug}`}>{n.title?.[lang]}</Link>
                  </li>
                ))
              ) : (
                <li>-</li>
...
const ArticlesItem = styled(motion.article)`
...
const ArticlesSummary = styled.p`
  color: ${({ theme }) => theme.colors.textSecondary};
  font-size: 1.08rem;

...
export default function ArticlesDetailSection() {
  ...
  return (
    ...
    >
      {/* Başlık */}
      <Title>{articles.title[lang]}</Title>
      ...
      {mainImage?.url && (
        <ImageSection>
          <MainImageFrame>
            <StyledMainImage
              src={mainImage.url}
              alt={articles.title[lang] || "Articles Title"}
              width={800}
              height={450}
              priority
            ...
            <Gallery>
              {images.map((img, i) => (
                ...
                >
                  <StyledThumbImage
                    src={img.url}
                    alt={`${articles.title[lang]} thumbnail ${i + 1}`}
                    width={168}
                    height={96}
                    $active={mainIndex === i}
                  />
      ...
      {/* Özet (Kısa Bilgi) */}
      {articles.summary && articles.summary[lang] && (
        <SummaryBox>
          <ReactMarkdown>
            {formatText(articles.summary[lang])}
          </ReactMarkdown>
        </SummaryBox>
      )}

      {/* Ana içerik */}
      {articles.content && articles.content[lang] && (
        <ContentBox>
          <ReactMarkdown>
            {formatText(articles.content[lang])}
          </ReactMarkdown>
        </ContentBox>
      )}
              ...
              <OtherCard key={item._id} as={motion.div} whileHover={{ y: -6, scale: 1.025 }}>
                ...
                <OtherTitleMini>
                  <Link href={`/articles/${item.slug}`}>
                    {item.title[lang] || "Articles Title"}
                  </Link>
                </OtherTitleMini>
              </OtherCard>
            ))}

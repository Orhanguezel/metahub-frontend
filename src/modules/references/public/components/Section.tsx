...
export default function ReferencesSection() {
  ...
  if (loading) {
    return (
      <Section>
        <Title>📰 {t("page.references.title")}</Title>
        <Grid>
          <Skeleton />
          <Skeleton />
          <Skeleton />
  ...
  if (!references || references.length === 0) {
    return (
      <Section>
        <Title>📰 {t("page.references.title")}</Title>
        <p>{t("page.references.noReferences", "Haber bulunamadı.")}</p>
      </Section>
    );
  }

  const latestReferences = references.slice(0, 3);
  ...
  return (
    ...
    >
      ...
      <Grid>
        {latestReferences.map((item: IReferences, index: number) => (
          <CardLink key={item._id} href={`/references/${item.slug}`} passHref>
            <Card
              as={motion.div}
            ...
            >
              <Content>
                <ReferencesTitle>{item.title?.[lang] || "-"}
                </ReferencesTitle>
              ...
              {item.images?.[0]?.url && (
                <StyledImage
                  ...
                  alt={item.title?.[lang] || "References"}
      ...
      <SeeAll href="/references">{t("page.references.all")}</SeeAll>
    </Section>
  );
}

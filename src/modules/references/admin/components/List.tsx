...
import { IReferences } from "@/modules/references";
      ...
      {references.map((item) => (
        <ReferencesCard key={item._id}>
          ...
          {item.images?.length > 0 ? (
            <ImageGrid>
              {item.images.map((img, i) => (
                <Image
                  key={i}
                  src={img.url}
                  alt={getMultiLang(item.title) || `references-${i}`}
                  loading="lazy"
                  width={150}
                  height={100}
                />
            ...
            <small>{t("references.no_images", "No images")}</small>
          )}

          <InfoLine>
            <strong>{t("references.author", "Author")}:</strong>{" "}
            {item.author || t("unknown", "Unknown")}
          </InfoLine>
          <InfoLine>
            <strong>{t("references.tags", "Tags")}:</strong>{" "}
            {item.tags?.length ? item.tags.join(", ") : t("none", "None")}
          </InfoLine>
          <InfoLine>
            <strong>{t("references.publish_status", "Published")}:</strong>{" "}
...
const ReferencesCard = styled.div`

...
import { deleteReferencesCategory } from "@/modules/references/slice/referencesCategorySlice";

interface ProductCategoryListPageProps {
  onAdd: () => void;
  onEdit: (category: ReferencesCategory) => void;
}
...
export default function ProductCategoryListPage({
  ...
  const handleDelete = (id: string) => {
    ...
    if (window.confirm(confirmMessage)) {
      dispatch(deleteReferencesCategory(id));
    }
  };
    ...
    <Wrapper>
      ...
      {loading ? (
        ...
        <Table>
          ...
          <tbody>
            {categories.map((cat: ReferencesCategory, i: number) => (
              <tr key={cat._id}>
                <td>{i + 1}</td>
                <td>{cat.name?.[lang] || "—"}</td>
                <td>{cat.slug}</td>

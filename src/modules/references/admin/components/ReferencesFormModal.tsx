...
import { clearReferencesMessages } from "@/modules/references/slice/referencesSlice"; // Mutlaka doğru import et!
...
export default function ReferencesFormModal({
  isOpen,
  onClose,
  editingItem,
  onSubmit,
  ...
  // --- STATE ---
  const { categories } = useAppSelector((state) => state.referencesCategory);
  const { successMessage, error } = useAppSelector((state) => state.references);
  ...
  return (
    <FormWrapper>
      <h2>
        {editingItem
          ? t("admin.references.edit", "Edit Article")
          : t("admin.references.create", "Create New Article")}
      </h2>

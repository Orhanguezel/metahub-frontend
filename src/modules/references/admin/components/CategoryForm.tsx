...
interface Props {
  ...
  onClose: () => void;
  onSubmit: (data: { name: CategoryName; description: CategoryDescription }, id?: string) => Promise<void>;
  loading?: boolean;
  error?: string | null;
...
export default function ReferenceCategoryForm({
  ...
  const [description, setDescription] = useState<CategoryDescription>({ tr: "", en: "", de: "" });
  ...
  return (
    <Form onSubmit={handleSubmit}>
      ...
      {languages.map((lng) => (
        <FieldGroup key={lng}>
          <label htmlFor={`name-${lng}`}>{t(`admin.name_${lng}`, `${lng.toUpperCase()} Name`)}</label>
          <input
            id={`name-${lng}`}
            type="text"
            value={name[lng]}

...
export default function FAQMultiForm({ onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    if (!isComplete) {
      toast.error(t("adminFaq.form.required", "Please fill in all fields in all languages."));
      return;
    }
    ...
    try {
      ...
      toast.success(t("adminFaq.messages.created", "FAQ created successfully."));
      ...
      toast.error(t("adminFaq.messages.error", "Error occurred while creating FAQ."));
    }
  };
  ...
  return (
    <Container>
      ...
      <CardGrid>
        {(Object.keys(languageLabels) as LangCode[]).map((lang) => (
          <LangCard key={lang}>
            <LangHeader>
              {languageLabels[lang]}{" "}
              {question[lang].trim() && answer[lang].trim() && <CheckMark>✓</CheckMark>}
            </LangHeader>
      ...
      <Input
        ...
        placeholder={t("adminFaq.form.categoryPlaceholder", "e.g., Shipping, Returns")}
      />

...
import { registerUser, clearAuthMessages } from "@/modules/users/slice/authSlice";
 ...
 useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!recaptchaToken) {
        toast.error(t("errors.recaptchaFailed", "reCAPTCHA validation failed."));
        return;
      }
      const payload = {
        name: form.username,
      ...
      // Stepper varsa: if (onSwitch) onSwitch({ step: "verifyEmail", payload: { email: form.email } });
    } catch (err: any) {
  toast.error(

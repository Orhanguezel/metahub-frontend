...
// --- Response Tipi (BE: { success, message, data }) ---
 ...
 * Not: Non-menu kalemleri için validator gereği unitPrice göndermen gerekir.
 * menuitem için unitPrice göndermen gerekmez (BE hesaplar).
 */
...
export const markOrderAsDeliveredAdmin = createAsyncThunk<ApiResponse<IOrder>, string>(
  "orders/markOrderAsDeliveredAdmin",
  async (id, thunkAPI) => {
...
// ADMIN: Siparişi sil
export const deleteOrderAdmin = createAsyncThunk<ApiResponse<{ id: string }>, string>(
  "orders/deleteOrderAdmin",
  async (id, thunkAPI) => await apiCall("delete", `/order/admin/${id}`, null, thunkAPI.rejectWithValue)
);
...
const getErrorMessage = (err: any): string => {
  ...
  if (err?.errors?.[0]?.msg) return err.errors[0].msg;
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchMyOrders.fulfilled, (state, action) => {
        ...
        // 404 durumunda reducer error’ı temiz kalsın
    ...
    builder
      ...
      .addCase(fetchAllOrdersAdmin.fulfilled, (state, action) => {
        state.loading = false;
        state.status = "succeeded";
        state.ordersAdmin = Array.isArray(action.payload?.data) ? action.payload.data : [];
      })
      ...
      .addCase(updateOrderStatusAdmin.fulfilled, (state, action) => {
        ...
        if (updated && Array.isArray(state.ordersAdmin)) {
          state.ordersAdmin = state.ordersAdmin.map((o) => (o._id === updated._id ? updated : o));
        }
      })
      ...
      .addCase(deleteOrderAdmin.fulfilled, (state, action) => {
        ...
        const id = action.payload?.id || (action.payload?.data && (action.payload.data as any).id);
        if (id) state.ordersAdmin = (state.ordersAdmin || []).filter((o) => o._id !== id);
      })

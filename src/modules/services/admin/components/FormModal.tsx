...
import type { ServicesCategory, IServices } from "@/modules/services/types";
...
export default function FormModal({
  ...
  const successMessage = useAppSelector((s) => s.services.successMessage);
  ...
  // --- Çok dilli alanlar (about patern)
  ...
  return (
    <Form onSubmit={handleSubmit}>
      <h2>{isEdit ? t("admin.services.edit", "Edit Services") : t("admin.services.create", "Create New Services")}</h2>

      <Row>
        <Col style={{ gridColumn: "span 2" }}>
          <Label>{t("admin.services.author", "Author")}</Label>
          <Input value={author} onChange={(e) => setAuthor(e.target.value)} required />
        </Col>
        <Col>
          <Label>{t("admin.services.durationMinutes", "Duration (min)")}</Label>
          <Input
            type="number"
            min={5}
            max={480}
        ...
        <Col>
          <Label>{t("admin.services.price", "Price")}</Label>
          <Input
            type="number"
            step="0.01"
            min={0}
      ...
      {editMode === "simple" ? (
        <>
          <Row>
            <Col style={{ gridColumn: "span 2" }}>
              <Label>{t("admin.services.title", "Title")} ({uiLang.toUpperCase()})</Label>
              <Input value={getTLStrict(title, uiLang)} onChange={(e) => setTitle(setTL(title, uiLang, e.target.value))} />
            </Col>
            <Col style={{ gridColumn: "span 2" }}>
              <Label>{t("admin.services.summary", "Summary")} ({uiLang.toUpperCase()})</Label>
              <TextArea rows={2} value={getTLStrict(summary, uiLang)} onChange={(e) => setSummary(setTL(summary, uiLang, e.target.value))} />
            </Col>
          </Row>
          <Row>
            <Col style={{ gridColumn: "span 4" }}>
              <Label>{t("admin.services.content", "Content")} ({uiLang.toUpperCase()})</Label>
              <TextArea rows={8} value={getTLStrict(content, uiLang)} onChange={(e) => setContent(setTL(content, uiLang, e.target.value))} />
            </Col>
          </Row>
        </>
      ) : (
      ...
      <Row>
        <Col style={{ gridColumn: "span 3" }}>
          <Label>{t("admin.services.tags", "Tags")}</Label>
          <Input placeholder="tag1, tag2, tag3" value={tags} onChange={(e) => setTags(e.target.value)} />
        </Col>
        <Col>
          <Label>{t("admin.services.category", "Category")}</Label>
          <Select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} required>
            <option value="">{t("admin.services.select_category", "Select a category")}</option>
            {catOpts.map((o) => (
      ...
      {/* Görseller */}
      <BlockTitle>{t("admin.services.image", "Images")}</BlockTitle>
      <ImageUploader
        existing={existing}
      ...
      <Row>
        <Col>
          <Label>{t("admin.services.publish_status", "Published?")}</Label>
          <CheckRow>

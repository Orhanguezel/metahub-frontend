// SparepartCategoryFormModal.tsx
"use client";
...
export default function SparepartCategoryFormModal({
  ...
  // Görseller (sıra + silinenler takibi)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // silinen mevcutlar (publicId + url birlikte gönder)
  ...
  return (
    <FormWrapper onSubmit={handleSubmit}>
      ...
      {editMode === "simple" ? (
        <>
          {SUPPORTED_LOCALES.map((lng) => (
            <div key={lng}>
              <Label>{t("admin.sparepartcategory.name", "Category Name")} ({lng.toUpperCase()})</Label>
              <Input
                value={getTLStrict(name, lng)}
                onChange={(e) => setName(setTL(name, lng, e.target.value))}
                required={lng === lang}
              />

              <Label>{t("admin.sparepartcategory.description", "Description")} ({lng.toUpperCase()})</Label>
              <TextArea
                value={getTLStrict(description, lng)}
                onChange={(e) => setDescription(setTL(description, lng, e.target.value))}
                required={lng === lang}
              />
            </div>
          ))}
      ...
      <BlockTitle>{t("admin.sparepartcategory.image", "Images")}</BlockTitle>
      <ImageUploader
      ...
      <Check>
        ...
        <span>{t("admin.sparepartcategory.isActive", "Category Active")}</span>
      </Check>

...
const tabs = [
  { key: "list", label: "admin.tabs.ensotekprod", fallback: "Ensotekprod" },
  { key: "create", label: "admin.tabs.create", fallback: "Add New" },
  { key: "categories", label: "admin.tabs.categories", fallback: "Categories" },
] as const;

export default function EnsotekprodTabs({ activeTab, onChange }: Props) {
  const {  t } = useI18nNamespace("ensotekprod", translations);

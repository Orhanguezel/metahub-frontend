...
export default function EnsotekprodList({
  ...
  if (ensotekprod.length === 0)
    return <Empty>{t("admin.ensotekprod.empty", "No ensotekprod available.")}</Empty>;
  ...
  return (
    <div>
      {ensotekprod.map((item) => (
        <EnsotekprodCard key={item._id}>
          ...
          {Array.isArray(item.images) && item.images.length > 0 ? (
            <ImageGrid>
              {item.images.map((img, i) => (
                <Image key={i} src={img.url} alt={`ensotekprod-${i}`} width={150} height={100} />
              ))}
            </ImageGrid>
          ) : (
            <small>{t("admin.ensotekprod.no_images", "No images")}</small>
          )}

          <InfoLine>
            <strong>{t("admin.ensotekprod.brand", "Brand")}:</strong> {item.brand}
          </InfoLine>

          <InfoLine>
            <strong>{t("admin.ensotekprod.price", "Price")}:</strong> €{item.price}
          </InfoLine>

          <InfoLine>
            <strong>{t("admin.ensotekprod.stock", "Stock")}:</strong> {item.stock}
          </InfoLine>

          <InfoLine>
            <strong>{t("admin.ensotekprod.tags", "Tags")}:</strong>{" "}
            {item.tags?.join(", ") || t("none", "None")}
          </InfoLine>

          <InfoLine>
            <strong>{t("admin.ensotekprod.publish_status", "Published")}:</strong>{" "}
          ...
          {(onEdit || onDelete || onTogglePublish) && (
            <ButtonGroup>
              ...
              {onTogglePublish && (
                ...
                >
                  {item.isPublished
                    ? t("admin.ensotekprod.unpublish", "Unpublish")
                    : t("admin.ensotekprod.publish", "Publish")}
                </ToggleButton>
              )}
            </ButtonGroup>
          )}
...
const EnsotekprodCard = styled.div`

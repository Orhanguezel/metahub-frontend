...
import { IMassage } from "@/modules/massage";
...
export default function MassageList({
  ...
  if (!Array.isArray(massage)) return null;
  if (massage.length === 0)
      ...
      {massage.map((item) => (
        <MassageCard key={item._id}>
          ...
          {item.images?.length > 0 ? (
            <ImageGrid>
              {item.images.map((img, i) => (
                <Image
                  key={i}
                  src={img.url}
                  alt={getMultiLang(item.title) || `massage-${i}`}
                  loading="lazy"
                  width={150}
                  height={100}
                />
            ...
            <small>{t("massage.no_images", "No images")}</small>
          )}

          <InfoLine>
            <strong>{t("massage.author", "Author")}:</strong>{" "}
            {item.author || t("unknown", "Unknown")}
          </InfoLine>
          <InfoLine>
            <strong>{t("massage.tags", "Tags")}:</strong>{" "}
            {item.tags?.length ? item.tags.join(", ") : t("none", "None")}
          </InfoLine>
          <InfoLine>
            <strong>{t("massage.publish_status", "Published")}:</strong>{" "}
            ...
            <ButtonGroup>
              ...
              {onTogglePublish && (
                <ToggleButton
                  ...
                  aria-label={
                    item.isPublished
                      ? t("massage.unpublish", "Unpublish")
                      : t("massage.publish", "Publish")
                  }
                >
                  {item.isPublished
                    ? t("massage.unpublish", "Unpublish")

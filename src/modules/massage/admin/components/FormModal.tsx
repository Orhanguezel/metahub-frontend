...
import { MassageCategory, IMassage } from "@/modules/massage/types";
...
export default function FormModal({
  ...
  // --- STATE ---
 // Sadece selector!
  const categories = useAppSelector((state) => state.massageCategory.categories);
  const successMessage = useAppSelector((state) => state.massage.successMessage);
...
if (price !== undefined) {
  ...
  return (
    <FormWrapper>
      <h2>
        {editingItem
          ? t("admin.massage.edit", "Edit Massage")
          : t("admin.massage.create", "Create New Massage")}
      </h2>
      <form onSubmit={handleSubmit}>
        {SUPPORTED_LOCALES.map((lng) => (
          <div key={lng}>
            <label htmlFor={`title-${lng}`}>
              {t("admin.massage.title", "Title")} ({lng.toUpperCase()})
            </label>
            <input
              id={`title-${lng}`}
              value={titles[lng]}
            ...
            <label htmlFor={`summary-${lng}`}>
              {t("admin.massage.summary", "Summary")} ({lng.toUpperCase()})
            </label>
            <textarea
              id={`summary-${lng}`}
              value={summaries[lng]}
            ...
            <label htmlFor={`content-${lng}`}>
              {t("admin.massage.content", "Content")} ({lng.toUpperCase()})
            </label>
            <textarea
              id={`content-${lng}`}
              value={contents[lng]}
        ...
        <label htmlFor="durationMinutes">{t("admin.massage.durationMinutes", "Duration")}</label>
        <input
          id="durationMinutes"
          type="number"
  ...
  max={480}
        />
        <label htmlFor="price">{t("admin.massage.price", "Price")}</label>
        <input
        name="price"
          id="price"
          type="number"
          ...
          step="0.01"
          min="0"
          placeholder={t("admin.massage.price_placeholder", "Enter price")}
        />

        <label htmlFor="author">{t("admin.massage.author", "Author")}</label>
        <input
          id="author"
          type="text"
          value={author}
        ...
        <label htmlFor="tags">{t("admin.massage.tags", "Tags")}</label>
        <input
          id="tags"
          type="text"
          value={tags}
        ...
        <label>{t("admin.massage.image", "Images")}</label>
        ...
        <label htmlFor="category">
          {t("admin.massage.category", "Category")}
        </label>
        <select
          id="category"
          value={category}
        ...
        >
          <option value="" disabled>
            {t("admin.massage.select_category", "Select a category")}
          </option>
          {categories.map((cat: MassageCategory) => (

...
import { FeedbacksCard, RevenueCard, StatsGrid, UsersCard } from "@/modules/dashboard";

export default function AdminDashboardPage() {
  const { t } = useTranslation("admin-dashboard");
  const dispatch = useAppDispatch();
  const [tab, setTab] = useState<"modules" | "stats" | "users" | "revenue" | "feedbacks">("modules");
  ...
  return (
    <Main>
      <TabBar>
        {tabs.map((item) => (
          <TabBtn key={item.key} $active={tab === item.key} onClick={() => setTab(item.key as any)}>
            {item.label}
          </TabBtn>
        ))}
      </TabBar>

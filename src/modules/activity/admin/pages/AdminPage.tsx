...
import { createActivity, updateActivity, deleteActivity, togglePublishActivity } from "@/modules/activity/slice/activitySlice";
import { createActivityCategory, updateActivityCategory } from "@/modules/activity/slice/activityCategorySlice";

import { ActivityForm, CategoryForm, CategoryListPage, List } from "@/modules/activity";
import { Modal } from "@/shared";
import type { IActivity, ActivityCategory } from "@/modules/activity/types";
...
export default function AdminActivityPage() {
  ...
  const activity = useAppSelector((state) =>
    Array.isArray(state.activity.activityAdmin) ? state.activity.activityAdmin : []
  );
  const loading = useAppSelector((state) => state.activity.loading);
  const error = useAppSelector((state) => state.activity.error);
  const categories = useAppSelector((s) => (s as any).activityCategory?.categories ?? []) as ActivityCategory[];
  ...
  const handleSubmit = async (formData: FormData, id?: string) => {
    if (id) await (dispatch(updateActivity({ id, formData }) as any)).unwrap().catch(() => {});
    else await (dispatch(createActivity(formData) as any)).unwrap().catch(() => {});
    setActiveTab("list");
  };
  const handleDelete = async (id: string) => {
    const confirmMsg = t("confirm.delete_activity", "Bu makaleyi silmek istediğinize emin misiniz?");
    if (confirm(confirmMsg)) await (dispatch(deleteActivity(id) as any)).unwrap().catch(() => {});
  };
  const handleTogglePublish = (id: string, isPublished: boolean) => {
    dispatch(togglePublishActivity({ id, isPublished: !isPublished }) as any);
  };
  const handleCategorySubmit = async (
    data: { name: Record<SupportedLocale, string>; description?: Record<SupportedLocale, string> },
    id?: string
  ) => {
    if (id) await (dispatch(updateActivityCategory({ id, data }) as any)).unwrap().catch(() => {});
    else await (dispatch(createActivityCategory(data) as any)).unwrap().catch(() => {});
  ...
  return (
    <PageWrap>
      ...
      <Header>
        <TitleBlock>
          <h1>{t("admin.title", "Activity Articles")}</h1>
          <Subtitle>{t("admin.subtitle", "Create, organize and publish your Activity content")}</Subtitle>
        </TitleBlock>
        <Right>
          <Counter aria-label="activity-count">{count}</Counter>
          <PrimaryBtn
            onClick={() => {
              setEditingItem(null);
              setActiveTab("create");
          ...
          {activeTab === "categories" && (
            <>
              <CategoryListPage
                ...
                onEdit={(category: ActivityCategory) => {

...
export default function ActivityCategoryForm({
  isOpen,
  onClose,
  editingItem,
  onSubmit,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!mainName.trim()) {
      alert(t("admin.activitycategory.name_required", "Category name required"));
      return;
    }
  ...
  return (
    <FormWrapper>
      <h2>
        {editingItem
          ? t("admin.activitycategory.edit", "Edit Activity Category")
          : t("admin.activitycategory.create", "Add New Activity Category")}
      </h2>
      <form onSubmit={handleSubmit}>
        {SUPPORTED_LOCALES.map((lng) => (
          <div key={lng}>
            <label>
              {t("admin.activitycategory.name", "Category Name")} ({lng.toUpperCase()})
            </label>
            <input
              type="text"
              value={name[lng]}
              ...
              required={lng === lang}
              placeholder={t("admin.activitycategory.name_placeholder", "Enter name")}
              autoFocus={lng === lang}
            />

            <label>
              {t("admin.activitycategory.description", "Description")} ({lng.toUpperCase()})
            </label>
            <textarea
              value={description[lng]}
              onChange={(e) => setDescription({ ...description, [lng]: e.target.value })}
              placeholder={t("admin.activitycategory.desc_placeholder", "Optional")}
            />
          </div>
        ))}

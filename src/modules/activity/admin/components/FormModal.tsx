...
export default function FormModal({
 ...
 // Sadece selector!
  ...
  const successMessage = useAppSelector((state) => state.activity.successMessage);
    ...
    <FormWrapper>
      <h2>
        {editingItem
          ...
          : t("admin.activity.create", "Create New Activity")}
      </h2>
      <form onSubmit={handleSubmit}>
        {SUPPORTED_LOCALES.map((lng) => (
          <div key={lng}>
            ...
            <label htmlFor={`summary-${lng}`}>
              {t("admin.activity.summary", "Summary")} ({lng.toUpperCase()})
            </label>
            <textarea
              id={`summary-${lng}`}
              value={summaries[lng]}
            ...
            <label htmlFor={`content-${lng}`}>
              {t("admin.activity.content", "Content")} ({lng.toUpperCase()})
            </label>
            <textarea
              id={`content-${lng}`}
              value={contents[lng]}
        ...
        <label htmlFor="category">
          {t("admin.activity.category", "Category")}
        </label>
        <select
          id="category"
          value={category}

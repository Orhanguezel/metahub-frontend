...
import type { IActivity, ActivityCategory, IActivityImage } from "@/modules/activity/types";
...
type Props = {
  ...
  initial?: IActivity | null;
  onSubmit: (formData: FormData, id?: string) => Promise<void> | void;
  onCancel: () => void;
  onAddCategory: () => void;
};

export default function ActivityForm({ categories, initial, onSubmit, onCancel, onAddCategory }: Props) {
  ...
  const originalExisting = useMemo<IActivityImage[]>(
    () => ((initial?.images || []) as IActivityImage[]),
    [initial?.images]
  );

...
export default function PortfolioForm({
  ...
  const { t, i18n } = useI18nNamespace("portfolio", translations);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasAnyTitle) {
      toast.warn(t("admin.portfolio.validation.title", "En az bir dilde başlık girmelisiniz."));
      return;
    }
  ...
  return isOpen ? (
    <FormCard role="form" aria-label={isEdit ? "Edit Portfolio" : "Create Portfolio"}>
      <Header>
        <h2>
          {isEdit
      ...
      <form onSubmit={handleSubmit}>
        {editMode === "simple" ? (
          <>
            <Row>
              <Col style={{ gridColumn: "span 2" }}>
                <Label>
                  {t("admin.portfolio.title", "Title")} ({lang.toUpperCase()})
                </Label>
                <Input
                  value={getTLStrict(title, lang)}
                  onChange={(e) => setTitle(setTL(title, lang, e.target.value))}
                  placeholder={t("admin.portfolio.title_ph", "Enter title")}
                />
              </Col>
              <Col style={{ gridColumn: "span 2" }}>
                <Label>
                  {t("admin.portfolio.summary", "Summary")} ({lang.toUpperCase()})
                </Label>
                <TextArea
                  rows={2}
                  value={getTLStrict(summary, lang)}
                  onChange={(e) => setSummary(setTL(summary, lang, e.target.value))}
                  placeholder={t("admin.portfolio.summary_ph", "Short summary")}
                />
              </Col>
            </Row>
            ...
            <Row>
              <Col style={{ gridColumn: "span 4" }}>
                <Label>
                  {t("admin.portfolio.content", "Content")} ({lang.toUpperCase()})
                </Label>
                <TextArea
                  rows={8}
                  value={getTLStrict(content, lang)}
                  onChange={(e) => setContent(setTL(content, lang, e.target.value))}
                  placeholder={t("admin.portfolio.content_ph", "Main content")}
                />
              </Col>
            </Row>
          </>
          ...
          <Row>
            <Col style={{ gridColumn: "span 4" }}>
              <JSONEditor
                label={t("admin.portfolio.multiLangJSON", "Title + Summary + Content (JSON)")}
        ...
        <Row>
          ...
          <Col style={{ gridColumn: "span 2" }}>
            <Label htmlFor="category">{t("admin.portfolio.category", "Category")}</Label>
            <Input
              id="category"
              type="text"
              value={category}
        ...
        {/* Görseller */}
        <BlockTitle>{t("admin.portfolio.image", "Images")}</BlockTitle>
        <ImageUploader

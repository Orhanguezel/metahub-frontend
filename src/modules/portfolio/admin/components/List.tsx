...
export default function PortfolioList({
  ...
  if (portfolio.length === 0)
    return <Empty>{t("portfolio.empty", "No portfolio available.")}</Empty>;
  ...
  {typeof item.category === "string"
    ...
    : t("none", "None")}
          ...
          {item.images?.length > 0 ? (
            <ImageGrid>
              {item.images.map((img, i) => (
                <Image
                  key={i}
                  src={img.url}
                  alt={getMultiLang(item.title) || `portfolio-${i}`}
                  loading="lazy"
                  width={150}
                  height={100}
                />
            ...
            <small>{t("portfolio.no_images", "No images")}</small>
          )}

          <InfoLine>
            <strong>{t("portfolio.author", "Author")}:</strong>{" "}
            {item.author || t("unknown", "Unknown")}
          </InfoLine>
          <InfoLine>
            <strong>{t("portfolio.tags", "Tags")}:</strong>{" "}
            {item.tags?.length ? item.tags.join(", ") : t("none", "None")}
          </InfoLine>
          <InfoLine>
            <strong>{t("portfolio.publish_status", "Published")}:</strong>{" "}

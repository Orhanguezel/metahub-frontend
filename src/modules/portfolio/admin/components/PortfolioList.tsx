...
import type { IPortfolio, TranslatedField } from "@/modules/portfolio/types";
...
export default function PortfolioList({
  ...
  const getCategoryLabel = (item: IPortfolio) =>
    typeof item.category === "string"
      ? item.category || t("none", "None")
      : getTL(item.category?.name, lang) || t("none", "None");

  const getThumb = (item: IPortfolio): string | undefined => {
        ...
        <Table>
          <thead>
            <tr>
              ...
              <th>{t("title", "Title")}</th>
              <th>{t("portfolio.category", "Category")}</th>
              <th>{t("portfolio.publish_status", "Published")}</th>
                    ...
                    <Row>
                      ...
                      {onTogglePublish && (
                        <Secondary onClick={() => onTogglePublish(it._id, it.isPublished)}>
                          {it.isPublished
                            ? t("portfolio.unpublish", "Unpublish")
                            : t("portfolio.publish", "Publish")}
                        </Secondary>
                      )}
                      {onDelete && (
          ...
          return (
            <Card key={it._id}>
              ...
              <CardActions>
                ...
                {onTogglePublish && (
                  <Secondary onClick={() => onTogglePublish(it._id, it.isPublished)}>
                    {it.isPublished ? t("portfolio.unpublish", "Unpublish") : t("portfolio.publish", "Publish")}
                  </Secondary>
                )}
                {onDelete && (

...
export default function BikesSection() {
  ...
  if (!bikes || bikes.length === 0) {
    return (
      <Section>
        ...
        <p>{t("page.noBike", "Henüz ürün yok.")}</p>
      </Section>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Grid>
        {latestBikes.map((item: IBikes, index: number) => (
          <CardWrapper key={item._id}>
            <CardLink href={`/bikes/${item.slug}`} passHref>
              <Card
                as={motion.div}
              ...
              >
                <Content>
                  <BikesTitle>{item.name?.[lang] || "-"}</BikesTitle>

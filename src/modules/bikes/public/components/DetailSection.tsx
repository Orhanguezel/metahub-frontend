...
export default function BikesDetailSection() {
  ...
  useEffect(() => {
    if (allProducts && allProducts.length > 0) {
      const found = allProducts.find((item:IBikes) => item.slug === slug);
      if (found) {
        dispatch(setSelectedBikes(found));
      } else {
        dispatch(fetchBikesBySlug(slug));
      }
    } else {
      dispatch(fetchBikesBySlug(slug));
    }
  ...
  const otherProducts = allProducts
    ?.filter((item: IBikes) => item.slug !== slug)
    .slice(0, 2);
    ...
    >
      ...
      {bike.images?.length > 0 && (
        <>
          ...
          <Thumbnails>
            {bike.images.map((img: IBikes["images"][number], idx: number) => (
              <ThumbWrapper
                key={idx}

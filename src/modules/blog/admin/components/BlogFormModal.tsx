...
export default function BlogFormModal({
  ...
  useEffect(() => {
    ...
    return () => { dispatch(clearCategoryMessages()); };
  }, [dispatch]);
  ...
  const handleImagesChange = useCallback((files: File[], removed: string[], current: string[]) => {
  ...
  return (
    <FormWrapper>
      ...
      <form onSubmit={handleSubmit}>
        {LANGUAGES.map((lng) => (
          <div key={lng}>
            <label>{t("admin.blog.title")} ({lng.toUpperCase()})</label>
            <input value={titles[lng]} onChange={(e) => setTitles({ ...titles, [lng]: e.target.value })} required />
            <label>{t("admin.blog.summary")} ({lng.toUpperCase()})</label>
            <textarea value={summaries[lng]} onChange={(e) => setSummaries({ ...summaries, [lng]: e.target.value })} required />
            <label>{t("admin.blog.content")} ({lng.toUpperCase()})</label>
            <textarea value={contents[lng]} onChange={(e) => setContents({ ...contents, [lng]: e.target.value })} required />
          </div>
        ))}
        ...
        <ImageUploadWithPreview max={5} defaultImages={existingImages} onChange={handleImagesChange} folder="blog" />
        <label>{t("admin.blog.category")}</label>
        <select value={category} onChange={(e) => setCategory(e.target.value)} required>
          <option value="">{t("admin.blog.select_category")}</option>
          {categories.map((cat) => (<option key={cat._id} value={cat._id}>{cat.name?.[currentLang]} ({cat.slug})</option>))}
        </select>
        <ButtonGroup>
          <button type="submit">{editingItem ? t("admin.update") : t("admin.create")}</button>

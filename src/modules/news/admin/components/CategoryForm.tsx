...
export default function CategoryForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem?._id) {
        ...
        await dispatch(createNewsCategory({ name, description })).unwrap();
      }
  ...
  return (
    <Form onSubmit={handleSubmit}>
      <h3>{editingItem ? t("Kategoriyi Düzenle") : t("Yeni Kategori")}</h3>
      ...
      {["tr", "en", "de"].map((lng) => (
        <div key={lng}>
          ...
          <input
            ...
            placeholder={`Kategori adı (${lng.toUpperCase()})`}
            required
          />
        </div>
      ))}

      <label htmlFor="desc">{t("Açıklama (opsiyonel)")}</label>
      <textarea
        id="desc"
        value={description}
        onChange={(e) => setDescription(e.target.value)}
        placeholder={t("Bu kategori neyle ilgili?")}
      />

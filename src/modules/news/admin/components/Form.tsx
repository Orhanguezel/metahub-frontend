...
import type { INews, NewsCategory, INewsImage } from "@/modules/news/types";
...
export default function NewsForm({ categories, initial, onSubmit, onCancel, onAddCategory }: Props) {
  const { t, i18n } = useI18nNamespace("news", translations);
  ...
  const originalExisting = useMemo<INewsImage[]>(
    () => ((initial?.images || []) as INewsImage[]),
    [initial?.images]
  );

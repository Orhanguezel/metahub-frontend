...
import { createNews, updateNews, deleteNews, togglePublishNews } from "@/modules/news/slice/newsSlice";
import { createNewsCategory, updateNewsCategory } from "@/modules/news/slice/newsCategorySlice";

import { NewsForm, CategoryForm, CategoryListPage, List } from "@/modules/news";
import { Modal } from "@/shared";
import type { INews, NewsCategory } from "@/modules/news/types";
...
export default function AdminNewsPage() {
  ...
  const news = useAppSelector((state) =>
    Array.isArray(state.news.newsAdmin) ? state.news.newsAdmin : []
  );
  const loading = useAppSelector((state) => state.news.loading);
  const error = useAppSelector((state) => state.news.error);
  const categories = useAppSelector((s) => (s as any).newsCategory?.categories ?? []) as NewsCategory[];
  ...
  const handleSubmit = async (formData: FormData, id?: string) => {
    if (id) await (dispatch(updateNews({ id, formData }) as any)).unwrap().catch(() => {});
    else await (dispatch(createNews(formData) as any)).unwrap().catch(() => {});
    setActiveTab("list");
  };
  const handleDelete = async (id: string) => {
    const confirmMsg = t("confirm.delete_news", "Bu makaleyi silmek istediğinize emin misiniz?");
    if (confirm(confirmMsg)) await (dispatch(deleteNews(id) as any)).unwrap().catch(() => {});
  };
  const handleTogglePublish = (id: string, isPublished: boolean) => {
    dispatch(togglePublishNews({ id, isPublished: !isPublished }) as any);
  };
  const handleCategorySubmit = async (
    data: { name: Record<SupportedLocale, string>; description?: Record<SupportedLocale, string> },
    id?: string
  ) => {
    if (id) await (dispatch(updateNewsCategory({ id, data }) as any)).unwrap().catch(() => {});
    else await (dispatch(createNewsCategory(data) as any)).unwrap().catch(() => {});
  ...
  return (
    <PageWrap>
      ...
      <Header>
        <TitleBlock>
          <h1>{t("admin.title", "News Articles")}</h1>
          <Subtitle>{t("admin.subtitle", "Create, organize and publish your News content")}</Subtitle>
        </TitleBlock>
        <Right>
          <Counter aria-label="news-count">{count}</Counter>
          <PrimaryBtn
            onClick={() => {
              setEditingItem(null);
              setActiveTab("create");

...
import type { IAbout, AboutCategory, IAboutImage } from "@/modules/about/types";
...
export default function AboutForm({ categories, initial, onSubmit, onCancel, onAddCategory }: Props) {
  const { t, i18n } = useI18nNamespace("about", translations);
  ...
  const originalExisting = useMemo<IAboutImage[]>(
    () => ((initial?.images || []) as IAboutImage[]),
    [initial?.images]
  );

...
const getArchives = (skill: ISkill[]) => {
  if (!Array.isArray(skill)) return [];
...
const getCategories = (skill: ISkill[], lang: SupportedLocale) => {
  if (!Array.isArray(skill)) return [];
...
export default function SkillPage() {
  ...
  const recentSkill = filteredSkill.slice(0, 4);
  const archives = useMemo(() => getArchives(skill || []), [skill]);
  const categories = useMemo(() => getCategories(skill || [], lang), [skill, lang]);
  ...
  if (!Array.isArray(skill) || skill.length === 0) {
    return (
      <PageWrapper>
        <NoSkill>{t("page.noSkill", "Hiç haber bulunamadı.")}</NoSkill>
      </PageWrapper>
    );
  }
  ...
  return (
    <PageWrapper>
      <MainGrid>
        {/* Sol: Skill Liste */}
        <LeftColumn>
          {filteredSkill.map((item: ISkill) => (
            <SkillItem key={item._id} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
              {item.images?.[0]?.url && (
                <MainImageWrap>
                  <Image
                    src={item.images[0].url}
                    alt={item.title?.[lang] || "Skill Image"}
                    width={780}
                    height={440}
                    style={{
                      width: "100%",
              ...
              <ReadMoreBtn href={`/skill/${item.slug}`}>
          ...
          <SidebarBox>
            ...
            <SidebarList>
              {recentSkill.length ? (
                recentSkill.map((n) => (
                  <li key={n._id}>
                    <Link href={`/skill/${n.slug}`}>{n.title?.[lang]}</Link>
                  </li>
                ))
              ) : (
                <li>-</li>
...
const SkillItem = styled(motion.article)`

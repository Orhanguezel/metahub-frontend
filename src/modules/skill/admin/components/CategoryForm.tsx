...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  editingItem: SkillCategory | null;
  onSubmit: (
...
export default function SkillCategoryForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!mainName.trim()) {
      alert(t("admin.skillcategory.name_required", "Category name required"));
      return;
    }
  ...
  return (
    <FormWrapper>
      <h2>
        {editingItem
          ? t("admin.skillcategory.edit", "Edit Skill Category")
          : t("admin.skillcategory.create", "Add New Skill Category")}
      </h2>
      <form onSubmit={handleSubmit}>
        {SUPPORTED_LOCALES.map((lng) => (
          <div key={lng}>
            <label>
              {t("admin.skillcategory.name", "Category Name")} ({lng.toUpperCase()})
            </label>
            <input
              type="text"
              value={name[lng]}
              ...
              required={lng === lang}
              placeholder={t("admin.skillcategory.name_placeholder", "Enter name")}
              autoFocus={lng === lang}
            />

            <label>
              {t("admin.skillcategory.description", "Description")} ({lng.toUpperCase()})
            </label>
            <textarea
              value={description[lng]}
              onChange={(e) => setDescription({ ...description, [lng]: e.target.value })}
              placeholder={t("admin.skillcategory.desc_placeholder", "Optional")}
            />
          </div>
        ))}

...
import { deleteSkillCategory } from "@/modules/skill/slice/skillCategorySlice";
...
export default function CategoryListPage({ onAdd, onEdit }: Props) {
  ...
  const { categories = [], loading, error } = useAppSelector((s)=> (s as any).skillCategory || {});

  const remove = async (id: string) => {
    if (!confirm(t("confirm.delete_category","Kategoriyi silmek istiyor musunuz?"))) return;
    await dispatch(deleteSkillCategory(id) as any).unwrap().catch(()=>{});
  };
      ...
      <TableWrap aria-busy={!!loading}>
        <Table>
          ...
          <tbody>
            ...
            {categories.map((c: SkillCategory)=>{

...
import type { SkillCategory } from "@/modules/skill/types";
...
export default function MultiUploadModal({ isOpen, onClose, onUpload }: MultiUploadModalProps) {
  ...
  // Kategori label’i
  const getLabel = (item: SkillCategory) => item.name?.[lang] || item.name?.en || Object.values(item.name)[0] || "–";
  ...
  const handleUpload = async () => {
    ...
    if (!files.length) {
      setError(t("skill.upload.no_files", "Dosya seçmelisiniz!"));
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <Box>
        <Title>{t("skill.bulk_upload", "Toplu Firma/Logo Yükle")}</Title>

        {/* Kategori seçici */}
        <Label>
          {t("skill.select_category", "Kategori Seçiniz")}
          <Select value={selectedCategory} onChange={handleCategoryChange}>
            <option value="">{t("skill.select_category_placeholder", "Seçiniz...")}</option>
            {categories.map((cat) => (
        ...
        {/* Dosya seçici */}
        <Label>
          {t("skill.select_files", "Firma logolarını seçiniz")}
          <Input
            type="file"
            accept="image/*"
            multiple
        ...
        {files.length > 0 && (
          <FileList>
            ...
            <ClearAll type="button" onClick={handleClear}>
              {t("skill.clear_all", "Tümünü Temizle")}
            </ClearAll>
          </FileList>
        )}
        ...
        <ActionRow>
          <ActionButton type="button" onClick={handleUpload} disabled={loading}>
            {loading ? t("skill.uploading", "Yükleniyor...") : t("skill.upload", "Yükle")}
          </ActionButton>
          <CancelButton type="button" onClick={onClose} disabled={loading}>
            {t("skill.cancel", "İptal")}
          </CancelButton>
        </ActionRow>
      </Box>
    </Modal>

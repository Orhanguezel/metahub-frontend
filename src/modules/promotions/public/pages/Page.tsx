...
import translations from "@/modules/promotions/locales/index";
import { Skeleton, ErrorMessage } from "@/shared";
import { motion } from "framer-motion";
import Link from "next/link";
import Image from "next/image";
import type { SupportedLocale } from "@/types/common";
import type { IPromotions } from "@/modules/promotions/types";
...
export default function PromotionsPage() {
  ...
  if (loading) {
    return (
      <PageWrapper>
        <PageTitle>{t("page.allPromotions", "Hakkımızda")}</PageTitle>
        <PromotionsGrid>
          {[...Array(3)].map((_, i) => (
            <Skeleton key={i} />
          ))}
  ...
  if (!promotions || promotions.length === 0) {
    return (
      <PageWrapper>
        <PageTitle>{t("page.allPromotions", "Hakkımızda")}</PageTitle>
        <EmptyMsg>
          {t("page.noPromotions", "Herhangi bir içerik bulunamadı.")}
        </EmptyMsg>
      </PageWrapper>
    );
  }
  ...
  return (
    <PageWrapper>
      <PageTitle>{t("page.allPromotions", "Hakkımızda")}</PageTitle>
      <PromotionsGrid>
        {promotions.map((item: IPromotions, index: number) => {
          const detailHref = `/promotions/${item.slug}`;
          return (
            <PromotionsCard
              key={item._id}
              as={motion.div}
...
const PromotionsCard = styled(motion.div)`

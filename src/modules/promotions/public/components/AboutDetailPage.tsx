...
export default function PromotionsDetailPage() {
  ...
  const { promotions, loading, error } = useAppSelector((s) => s.promotions);
  const item = promotions?.find((a: IPromotions) => a._id === id);
  ...
  if (error || !item) {
    return (
      <PageWrapper>
        <ErrorMessage message={error || t("page.noPromotions", "İçerik bulunamadı")} />
      </PageWrapper>
    );
  }
  ...
  if (contentHtml.startsWith("<p>")) {
    ...
    // Plain text ise ilk satırı ayır
  ...
  return (
    <PageWrapper>
      ...
      <HeroModern>
        ...
        <InfoCol>
          <HeroTitle>
            {item.title?.[lang] || item.title?.tr || item.title?.en || "—"}
          </HeroTitle>
          <HeroSummary>
            {item.summary?.[lang] || item.summary?.tr || item.summary?.en || "—"}
          </HeroSummary>
        </InfoCol>
      </HeroModern>
      ...
      {galleryImgs.length > 0 && (
        <GallerySection>
          <GalleryTitle>{t("promotions.gallery", "Galeri")}</GalleryTitle>
          <MasonryGallery
            breakpointCols={{ default: 3, 1024: 2, 640: 1 }}
            className="masonry-grid"
            columnClassName="masonry-column"
          >
            {galleryImgs.map((img: IPromotions["images"][number], i: number) => (
              <ImgWrap key={img._id || i} onClick={() => { setModalIdx(i); setOpenModal(true); }}>
                <StyledImage
                  src={img.url}
                  alt={`promotions-img-${i + 1}`}
                  width={600}
                  height={400}
                  loading="lazy"
            ...
            >
              <ModalImageWrap>
                <Image
                  ...
                  alt={`promotions-modal-img-${modalIdx + 1}`}
                  width={1100}
                  height={700}
                  style={{
                    maxWidth: "90vw",
...
// Global animasyon ekleme
if (typeof window !== "undefined" && !document.getElementById("promotions-theme-animations")) {
  const style = document.createElement('style');
  style.id = "promotions-theme-animations";

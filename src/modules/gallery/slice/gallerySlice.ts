...
// ✅ Kategorileri çek (örneğin /gallery/categories endpointinden)
...
// ✅ Belirli kategoriye ait içerikleri çek
export const fetchGalleryItemsByCategory = createAsyncThunk(
  "gallery/fetchGalleryItemsByCategory",
  async (category: string, thunkAPI) =>
    await apiCall(
      "get",
      `/gallery/published/category/${category}`,
      null,
      thunkAPI.rejectWithValue
    )
);
...
const gallerySlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(fetchGalleryCategories.pending, loadingReducer)
      .addCase(fetchGalleryCategories.fulfilled, (state, action) => {
        state.loading = false;
        state.categories = action.payload.data || [];
      })
      .addCase(fetchGalleryCategories.rejected, errorReducer);

      builder
      .addCase(fetchGalleryItemsByCategory.pending, loadingReducer)
      .addCase(fetchGalleryItemsByCategory.fulfilled, (state, action) => {
        state.loading = false;
        state.items = action.payload.data; // tüm array gelsin
      })
      
      
      .addCase(fetchGalleryItemsByCategory.rejected, errorReducer);

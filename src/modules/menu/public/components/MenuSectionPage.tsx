...
export default function MenuSection({ vm, uiLang, t, isLoading, catDict }: Props) {
  const tr = useMemo(() => (typeof t === "function" ? t : (k: string, d?: string) => d ?? String(k)), [t]);
  ...
  const firstSelectable = sideCats.find((c) => c.count > 0)?.id || sideCats[0]?.id || null;
  const [activeCat, setActiveCat] = useState<string | null>(firstSelectable);
  useEffect(() => { setActiveCat(firstSelectable); }, [firstSelectable]);

  const itemsByCategory = useMemo(() => vm?.itemsByCategory ?? new Map<string, IMenuItem[]>(), [vm?.itemsByCategory]);
  const currentItems = useMemo(() => (activeCat ? (itemsByCategory.get(activeCat) || []) : []), [activeCat, itemsByCategory]);
  ...
  return (
    <section>
      <Hero>
        ...
        <div className="crumbs" aria-label="breadcrumb">Home / Menu</div>
      </Hero>
        ...
        <RightCol>
          <Toolbar>
            <span className="results">
              {total > 0 ? `Showing 1–${total} of ${total} results` : tr("noItems", "Gösterilecek ürün bulunamadı.")}
            </span>
...
const Hero = styled.header`
  /* gradient arka plan (istersen kaldırabilirsin) */
  ...
  /* ⚠️ Boşluk yapan min-height kaldırıldı */
  /* Mobilde/desktopta içerik kadar yükseklik */

  .crumbs{
    position: relative; z-index: 2;     /* görsel her zaman arkada */

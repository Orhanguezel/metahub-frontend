...
export default function DetailsModal({
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(
        replyToComment({ id: comment._id!, text: reply })
      ).unwrap();
      onClose();
    } finally {
      setSending(false);
    ...
    <ModalOverlay onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        ...
        <h2 style={{ marginTop: 0 }}>
          {t("details.title", "Yorum Detayları")}
        </h2>
        ...
        <p>
          <b>{t("email", "E-posta")}:</b> {email}
        </p>
        <p>
          <b>{t("contentType", "İçerik Türü")}:</b> {comment.contentType}
        </p>
        <p>
        ...
        <p>
          <b>{t("label", "Başlık")}:</b> {comment.label || "-"}
        </p>
        <p>
          <b>{t("text", "İçerik")}:</b>
          <br />

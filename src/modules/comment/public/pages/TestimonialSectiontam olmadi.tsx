...
const slideVariants = {
  ...
  center: {
    ...
    transition: {
      duration: 0.9,
      ease: [0.44, 0.11, 0.58, 0.98], // ultra smooth, neredeyse linear gibi
    },
  },
  exit: (dir: number) => ({
    x: dir > 0 ? -400 : 400,
...
export default function TestimonialSection() {
  ...
  // Otomatik kaydırma (soft infinite loop)
  useEffect(() => {
    if (testimonials.length <= 3) return;
    timerRef.current = setTimeout(() => {
      slideTo(activeIdx + 1, 1);
    }, 4800);
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (dx > 40) slideTo(activeIdx - 1, -1);
    if (dx < -40) slideTo(activeIdx + 1, 1);
    startX.current = null;
  };
  ...
  const nextSlide = () => slideTo(activeIdx + 1, 1);
  const prevSlide = () => slideTo(activeIdx - 1, -1);
  ...
  // 3'lü makara slider pozisyonları
  const getDisplayTestimonials = () => {
    const n = testimonials.length;
    if (n < 1) return [];
    return [-1, 0, 1].map((offset) => testimonials[(activeIdx + offset + n) % n]);
  };
  ...
  const displayCards = getDisplayTestimonials();
        ...
        <Row>
          <AnimatePresence initial={false} custom={direction}>
            {displayCards.map((card, i) => {
              if (!card) return <EmptyCard key={i} />;
              const pos = i - 1;

...
export default function TeamSection() {
  ...
  if (!Array.isArray(team) || team.filter(Boolean).length === 0) {
    return (
      <Section>
        <TeamGrid>
          <Left>
            <MainTitle>{t("page.team.allTeam", "Hakkımızda")}</MainTitle>
            <Desc>{t("team.team.empty", "Hakkında içeriği bulunamadı.")}</Desc>
          </Left>
        </TeamGrid>
      </Section>
    );
  ...
  const validTeam = team.filter((item) => !!item && typeof item === "object");
  const main =
    validTeam.find((x) => !!x?.slug && x.slug === "ensotek-su-sogutma-kuleleri") ||
    (validTeam.length > 2 ? validTeam[2] : validTeam[0]) ||
    {};
  ...
  return (
    ...
    >
      <TeamGrid>
        {/* SOL BLOK */}
        <Left>
          <MinorTitle>{t("page.team.minorTitle", "HAKKIMIZDA")}</MinorTitle>
          <MainTitle>
            {main?.title?.[lang] ||
              main?.title?.en ||
              t("page.team.title", "Ensotek Hakkında")}
          </MainTitle>
          <Desc>
          ...
          <SeeAllBtn href="/team">
            {t("page.team.all", "Daha Fazla Bilgi")}
          </SeeAllBtn>
        </Left>
        ...
        <Right>
          {main?.slug && main?.images?.[0]?.url && (
            <MainImageWrap as={Link} href={`/team/${main.slug}`}>
              <MainImage
                src={main.images[0].url}
                alt={main.title?.[lang] || "Team"}
                width={330}
                height={210}
          ...
          <StackedImages>
            {rightImages.map((item, idx) =>
              item?.images?.[0]?.url && item?.slug ? (
                <StackedImageLink key={item.slug} href={`/team/${item.slug}`}>
                  <StackedImage
                    src={item.images[0].url}
                    alt={item.title?.[lang] || "Team"}
                    width={135}
                    height={90}
                    style={{
                      objectFit: "cover",

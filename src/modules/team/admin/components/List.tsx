      ...
      {team.map((item) => (
        <TeamCard key={item._id}>
          ...
          {item.images?.length > 0 ? (
            <ImageGrid>
              {item.images.map((img, i) => (
                <Image
                  key={i}
                  src={img.url}
                  alt={getMultiLang(item.title) || `team-${i}`}
                  loading="lazy"
                  width={150}
                  height={100}
                />
            ...
            <small>{t("team.no_images", "No images")}</small>
          )}

          <InfoLine>
            <strong>{t("team.author", "Author")}:</strong>{" "}
            {item.author || t("unknown", "Unknown")}
          </InfoLine>
          <InfoLine>
            <strong>{t("team.tags", "Tags")}:</strong>{" "}
            {item.tags?.length ? item.tags.join(", ") : t("none", "None")}
          </InfoLine>
          <InfoLine>
            <strong>{t("team.publish_status", "Published")}:</strong>{" "}

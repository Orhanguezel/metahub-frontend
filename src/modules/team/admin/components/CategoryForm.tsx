...
import type { TeamCategory } from "@/modules/team/types";

interface Props {
  isOpen: boolean;
  onClose: () => void;
  editingItem: TeamCategory | null;
  onSubmit: (
...
export default function TeamCategoryForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!mainName.trim()) {
      alert(t("admin.teamcategory.name_required", "Category name required"));
      return;
    }
  ...
  return (
    <FormWrapper>
      <h2>
        {editingItem
          ? t("admin.teamcategory.edit", "Edit Team Category")
          : t("admin.teamcategory.create", "Add New Team Category")}
      </h2>
      <form onSubmit={handleSubmit}>
        {SUPPORTED_LOCALES.map((lng) => (
          <div key={lng}>
            <label>
              {t("admin.teamcategory.name", "Category Name")} ({lng.toUpperCase()})
            </label>
            <input
              type="text"
              value={name[lng]}
              ...
              required={lng === lang}
              placeholder={t("admin.teamcategory.name_placeholder", "Enter name")}
              autoFocus={lng === lang}
            />

            <label>
              {t("admin.teamcategory.description", "Description")} ({lng.toUpperCase()})
            </label>
            <textarea
              value={description[lng]}
              onChange={(e) => setDescription({ ...description, [lng]: e.target.value })}
              placeholder={t("admin.teamcategory.desc_placeholder", "Optional")}
            />
          </div>
        ))}
